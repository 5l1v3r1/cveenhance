...
/**
 * This class should represent a part of a floating text e.g. a Token / combined Tokens.
 * Furthermore a Snippet should represents a logical unit e.g. a software name, a version number or a stopword.
...
public class Snippet {
	...
	private boolean osExt = false;
	private boolean stopword = false;
	private boolean concatword = false;
	private boolean seperator = false;
	private boolean nameStart = false;
	private boolean versionStart = false;
	private boolean versionEnd = false;
	*/
	...
	public boolean isMoreThanTwoChar() {return word;}
	public boolean isPossibleVersion() {return possibleVersion;}
	public boolean startsWithBigLetter() {return bigLetter;}
	public boolean isNotUsed() {return notToUse;}
	public boolean isUsed() {return used;}
	public boolean isVersion() {return version;}
	public boolean isOs() {return os;}
	public boolean isOsExt() {return osExt;}
	public boolean isStopword() {return stopword;}
	public boolean isConcatWord() {return concatword;}
	public boolean isSeperator() {return seperator;}
	public boolean isNameStartKeyword() {return nameStart;}
	public boolean getVersionStart() {return versionStart;}
	public boolean getVersionEnd() {return versionEnd;}
	public boolean isSingleToken() {return singleToken;}
	public boolean isPartOfName() {return name;}
	*/
	
	
	public int 	   getTokenValue(){return tokenValue;}	
	public boolean islogicalEnd(){return logicalEnd||!hasNext();}
	public boolean islogicalStart(){return logicalStart;}
...
//  Init:
	...
	public void init(){
		...
		try {			
			...
			if (prev!=null && lcheck("["+createRegexpFromStrings(konfig.seperatingChar )+"]")) prev.setFeature("logicalend", true);
			if(getFeatureValue("version")) System.out.println(getText());
			
			if(hasPrev()==false) setFeature("logicalstart", true);
			else{
				if(prev.islogicalEnd())setFeature("logicalstart", true);
				else setFeature("logicalstart", false);			
			}
		...
		/*
		// TODO: Check the efficiency of all regex
		comma=			lcheck(".*,");
		if(comma){ setText(text.substring(0, text.length()-1)); } // also Point chars
		possibleVersion=lcheck(".*\\d.*");
		word=			(text.length()>=3);
		bigLetter=		check("[A-Z]+.*");
		version=		keywordCheck(lowerCaseText, konfig.versionKeywords)||(lcheck("[\\d]+[\\p{Punct}\\w]*")&&!(lcheck(""))); 
		
		//TODO: Insert Regex for Dates
		os=				keywordCheck(lowerCaseText, konfig.osKeywords);
		osExt=			keywordCheck(lowerCaseText, konfig.osExctentions);
		stopword=		keywordCheck(lowerCaseText, konfig.stopWords);
		concatword=		keywordCheck(lowerCaseText, konfig.concatWords);
		seperator=		keywordCheck(lowerCaseText, konfig.seperatingWord);
		nameStart=		keywordCheck(lowerCaseText, konfig.softwareNameStartWords);
		versionStart=	keywordCheck(lowerCaseText, konfig.softwareNameStopWords);
		versionEnd=		keywordCheck(lowerCaseText, konfig.softwareVersionEnd);
		if(comma)setLogicalEnd(true); // Point Chars also have to be mentioned
		if(lcheck(".+["+createRegexpFromStrings(konfig.seperatingChar)+"]")) setLogicalEnd(true);
		else if (prev!=null && lcheck("["+createRegexpFromStrings(konfig.seperatingChar )+"]")) prev.setLogicalEnd(true);
		
		if(version) System.out.println(getText());
		
		if(hasPrev()==false) logicalStart=true;
		else{
			if(prev.islogicalEnd())logicalStart=true;
			else logicalStart=false;			
		}	
		*/
	}
...
//  Methods:
	...
	private void setFeature(String featureName, boolean newValue) throws Exception{
		...
		if(features.size()!=defaultVector.size()) throw new Exception("Feature \""+featureName+"\" is not defined in konfig!");
	}
	
	public String createRegexpFromStrings(String[] keywords, String seperator){
		String regexp="";
		if(keywords.length>=1) regexp+=Pattern.quote(keywords[0]);
	...
	private boolean keywordCheck(String checkword, String[] keywordList){ 
		...
		for(int i=0; i<keywordList.length; i++){
			if (checkword.equalsIgnoreCase(keywordList[i])) return true;
		}
		return false;
	}
	...
	public void mergeWithNextSnippet(){
		...
		if(hasNext()){
			...
			/*
			comma = next.hasComma();
			word = word ||next.isMoreThanTwoChar();
			possibleVersion = possibleVersion||next.isPossibleVersion();
			bigLetter = bigLetter||next.startsWithBigLetter();
			notToUse = (notToUse && next.isNotUsed());
			used = used || next.isUsed();
			version = version || next.isVersion();
			os = os || next.isOs();
			osExt = osExt || next.isOsExt();
			stopword = stopword || next.isStopword();
			concatword = concatword || next.isConcatWord();
			seperator = seperator || next.isSeperator();
			nameStart = nameStart || next.isNameStartKeyword();
			versionStart = versionStart || next.versionStart;
			versionEnd = versionEnd || next.getVersionEnd();
			*/
...
//	}
	...
	public String logicalType(){
		if (logicalUnit==null||!logicalUnit.isValid()) return null;
		else return logicalUnit.type();
	}
	
	public boolean isLogicalType(String type){
		if (logicalUnit==null||!logicalUnit.isValid()) return false;
		else if (logicalUnit.type()==type) return true;
		return false;
	}
	...
	public void melt() {
		...
		for(int i=0;i<meltingNumber;i++)mergeWithNextSnippet();
		//adjustSnippetFeatures();
	}
	
	/*
	private void adjustSnippetFeatures() {
	...
	*/
	...
	public boolean condition(String requestCondition) throws Exception{
		...
		for(String condition : conditions){
			...
			if(indicator=="!"){
				...
				if(!getFeatureValue(condition)) return false;
			}
			else if(indicator=="-"){
				condition=condition.substring(1);
				if(getFeatureValue(condition)) return false;
				...
				if(!logicalUnit.isType(condition)) return false;
			}
			else{
	...
	private int meltable() {
		...
		while(!curSnip.islogicalEnd() && correspondingConditions.size()!=0){
			...
			if(correspondingConditions.size()!=0)meltingNumber++;
